...
public class InventorySlot : VisualElement
{
    ...
    public virtual void HoldItem(ItemInstance item)
    {
        if (item == null) 
        {
            return;
        ...
        _item = item;
        _icon.style.backgroundImage = new StyleBackground(item.Data.Icon);


        _rarityClass = item.Data.RarityClass;
        _slotRoot.AddToClassList(_rarityClass);

    }

    public virtual ItemInstance DropItem()
    {
        if (_item == null) 
        {
            return null;
